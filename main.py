            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if extracted:
                        ...
                        # Convert to numeric where possible, keep original string on failure
                        df_extracted["Value_num"] = pd.to_numeric(df_extracted["Value"], errors="coerce")
                        
                        # Create display column: formatted number or original string
                        df_extracted["Display Value"] = df_extracted.apply(
                            lambda row: f"{row['Value_num']:.2f}" if pd.notna(row['Value_num']) else row["Value"],
                            axis=1
                        )

                        # Final dataframe to show
                        df_display = df_extracted[["Test", "Display Value"]].rename(columns={"Display Value": "Value"})
